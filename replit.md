...
- **Constellation Alerts**: Per-source 4D state vector (Capability/Integrity/Usability/Trust) that updates with reflections. Reports clearing a threshold trigger narrated alerts via Telegram.
...
- **Telegram Q&A**: Long-polling loop answers inbound text messages from whitelisted chat IDs using `gpt-4o-mini` with web search, adopting a narrated council persona.
...
- **Higgs Phase Transition Diagnostic (May 2026)**: Per-step symmetry-breaking observable plumbed through the biomimetic solver. `routes/ephemeroi/higgs.ts` defines per-variable mass (Δunsat on flip, restored), reservoir-samples K vars per snapshot, and computes `fieldStrength` (mean mass), `fieldVariance`, and `orderParameter = fieldVariance / (|fieldStrength| + ε)`. `HiggsLogger` streams snapshots from the solve loop without mutating the assignment; every biomimetic run auto-persists one row to `ephemeroi_higgs_runs` (outcome label `solved`/`stuck_soft`/`stuck_hard`, snapshot trajectory in jsonb, seed + dimensions for reproducibility). `analyzeHiggsRuns` builds per-outcome mean-OP profiles, detects the OP-threshold crossing step per outcome, and surfaces `earlyWarningStep` — the first step at which `solved` and `stuck_hard` profiles diverge by more than 1.0. HTTP: `GET /ephemeroi/higgs/runs?outcome=&limit=`, `GET /ephemeroi/higgs/runs/{id}` (full trajectory), `POST /ephemeroi/higgs/analyze`. UI: `/higgs` page with cross-run analysis chart (mean OP per outcome with threshold + early-warning markers), filterable run list, and a per-run drilldown showing OP and field-strength traces. THEORY.md ⭐ 10 documents the theory.
...
- **Ollama**: Optional local LLM for narration.